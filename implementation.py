...
class AutoTrader(BaseAutoTrader):
    def __init__(self, loop: asyncio.AbstractEventLoop):
        ...
        self.activevolb = self.activevols = 0
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        #Volume Calculation
        #Max Order Size
        #print('self pos is', self.position)
        self.ordmaxb = min(65, (100 - self.position) * 0.5)
        self.ordmaxs = min(65, (100 + self.position) * 0.5)
        #print(self.ordmaxb, 'ordmaxb')
        #print(self.ordmaxs, 'ordmaxs')
        #BUY VOL
        if self.position <= 0:
            self.buyvol = min(100, math.floor(self.ordmaxb - self.position * 0.625))
            #print('buyvol under 0', self.buyvol)
        if self.position  > 0:
            if self.position >= 90:
                self.buyvol = 0
            else:
                self.buyvol =  min(45, math.floor((self.ordmaxb) * np.exp(self.shape*self.ordmaxb)))
            #print('buyvol', self.buyvol)
        
        #SELL VOL
        if self.position >= 0:
            self.sellvol = min(100, math.floor(self.ordmaxs + self.position * 0.625))
        if self.position < 0:
            if self.position <= -90:
                self.sellvol = 0
            else:
                self.sellvol = min(45, math.floor((self.ordmaxs) * np.exp(self.shape*self.ordmaxs)))
            #print('sellvol less than 0', self.sellvol)
        ...
        if instrument == Instrument.FUTURE:
            ...
            #PRINT
            print(self.order_ids, ' Bid', best_bid, ' Ask', best_ask, ' BuyVol', self.buyvol, ' SellVol', self.sellvol, ' Position', self.position)

            #Active Volume Calc
            self.activevolb = self.position + self.buyvol
            self.activevols = self.position - self.sellvol
            ...
            if self.bid_id == 0 and best_bid != 0 and self.activevolb < 89 and self.buyvol > 0:
            ...
            if self.ask_id == 0 and best_ask != 0 and self.activevols > -89 and self.sellvol > 0:
