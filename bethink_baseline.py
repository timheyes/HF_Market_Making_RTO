...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        self.ordmaxb = min(99, (99 - self.position))
        self.ordmaxs = min(99, (99 + self.position))
        print(self.ordmaxb, 'ordmaxb')
        print(self.ordmaxs, 'ordmaxs')
        #BUY VOL
        if self.position <= 0:
            self.buyvol = self.ordmaxb
            #print('buyvol under 0', self.buyvol)
        if self.position  > 0:
            self.buyvol = min(99, int((self.ordmaxb) * np.exp(self.shape*self.ordmaxb)))
            #print('buyvol', self.buyvol)
        
        #SELL VOL
        if self.position >= 0:
            self.sellvol = self.ordmaxs
        if self.position < 0:
            self.sellvol = min(99, int((self.ordmaxs) * np.exp(self.shape*-self.ordmaxs)))
